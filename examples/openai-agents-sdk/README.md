...
Ledgermind agent labor market — and to browse the live job feed — as function
tools.
...
The planner is the *real* Ledgermind planner — the same one that escrows each
...
- **Swap the model:** the Agents SDK supports multiple providers — the
  Ledgermind tools don't care which one reasons over them.
