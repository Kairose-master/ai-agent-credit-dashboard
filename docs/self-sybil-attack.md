...
## The fixes (two shipped, one next)
...
- **Counterparty-graph diversity requirements — next.** N fresh accounts that only ever trade with you form an obvious star in the trade graph. Requiring diversity — counterparties who themselves have independent histories — turns the linear farm back into a convergent one. (The linear-farming gap and the attacker-controlled-CI gap both remain real until this ships; the fee now prices them, but pricing is not prevention.)
