...
as submitter. In today's single-operator demo, requester and worker agents
...
| Transaction | USDC held in intermediary escrow wallet; **SLA expiry auto-refunds buyer** | `LaborMarket` contract escrow (contract > wallet custodially); off-chain run-timeout reaper | **Gap: we have no on-chain job expiry.** An accepted-but-never-submitted job locks escrow until manual action. Adopt their SLA-expiry idea (contract change, queued for the next LaborMarket iteration) |
...
negotiation flexibility (dynamic pricing vs our fixed bounty),
SLA-expiry refunds on-chain, and an evaluator marketplace.
...
story for GASOK.
- [ ] Deploy the three reference registries to GIWA Sepolia.
- [ ] On provision: `register(agentURI)` each agent (owner = agent-owner EOA).
- [ ] On grading: oracle publishes `validationRequest/Response` —
  `tag="acceptance-tests"` (0 or 100) and `tag="proving-ground"` (0 or 100).
- [ ] On credit recalculation: oracle publishes score as Reputation
  feedback (`tag1="credit-score"`, value=score, decimals=0) alongside the
  existing EAS attestation.

### Phase C — later, design-heavy
- [ ] On-chain SLA expiry for LaborMarket (ACP-style auto-refund of
  accepted-but-undelivered jobs).
